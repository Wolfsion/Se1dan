...
import com.lavson.se1dan.service.UserAnswerService;
...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
    ...
    public void validUserAnswer(UserAnswer userAnswer, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "appId 为空或非法");
            ThrowUtils.throwIf(id == null || id <= 0, ErrorCode.PARAMS_ERROR, "id 为空或非法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (appId != null) {
